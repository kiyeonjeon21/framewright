 ...
 * the core bundle (everything exported from `@framewright/core`) to window.__TheatreJS_CoreBundle.
 */
function registerCoreBundle() {
  ...
  if (typeof existingBundle !== 'undefined') {
    if (
      ...
      is bundling `@framewright/core` multiple times, but, there are legitimate scenarios where a user may want
      to include multiple instances of `@framewright/core` on the same page.
      ...
      */
      throw new Error(
        `It seems that the module '@framewright/core' is loaded more than once. This could have two possible causes:\n` +
          `1. You might have two separate versions of Theatre.js in node_modules.\n` +
          `2. Or this might be a bundling misconfiguration, in case you're using a bundler like Webpack/ESBuild/Rollup.\n\n` +
          `Note that it **is okay** to import '@framewright/core' multiple times. But those imports should point to the same module.`,
      )
    } else {
      throw new Error(
        `The variable window.${globalVariableNames.coreBundle} seems to be already set by a module other than @framewright/core.`,
      )
    }
  }
