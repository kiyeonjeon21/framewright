...
As the names imply, one includes both `@framewright/studio` and `@framewright/core`, while the other is a minified version of `@framewright/core`.
...
<script>
  // here, core is equal to `import * as core from '@framewright/core`
  const core = Theatre.core
  // here, studio is equal to `import studio from '@framewright/studio`.
